...
from app.auth.forms import RegistrationForm,LoginForm,EditProfileForm,EmptyForm,PostForm,ResetPasswordForm,ResetPasswordRequestForm
...
def register():
    ...
    if form.validate_on_submit():
        user = Client(username=form.username.data, email=form.email.data)
...
def user(username):
    form = EmptyForm()
    user = Client.query.filter_by(username=username).first_or_404()
    page = request.args.get('page', 1, type=int)
...
def reset_password(token):
    if current_user.is_authenticated:
        return redirect(url_for('index'))
    user = Client.verify_reset_password_token(token)
    if not user:
        return redirect(url_for('index'))
